ROOMS = {
    ...
    'garden': {
        'description': 'Умиротворяющий цветущий сад. В фонтане журчит вода, щебечут птички.',
        'exits': {'west': 'hall', 'east': 'crypt', 'south': 'trap_room'},
        'items': {'chest', 'apple'},
        'puzzle': None
    },
    'crypt': {
        'description': (
            'Мрачная крипта, освещенная дрожащими факелами. На обветшалом столе лежит череп. '
            'Вокруг стоят статуи древних воинов. '
            'За одной из статуй виднеется щель - возможно, это потайной проход?'
            ),
        'exits': {'west': 'garden', 'north': 'secret_room'},
        'items': {'skull'},
        'puzzle': ('Я когда-то был домо для мыслей и чувств. Теперь я - пустая оболочка. Что я?', 'череп')
    },
    'secret_room': {
        'description': 'Крохотная келья, на постаменте из черного мрамора лежат драгоценности и карта.',
        'exits': {'south': 'crypt'},
        'items': {'map'},
        'puzzle': ('Я жду света, чтобы явиться. Что я?', 'Тень')
    }
}
